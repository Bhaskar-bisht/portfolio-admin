...
import { BooleanInput, Create, DateInput, Edit, required, SimpleForm, TextInput, useGetList } from "react-admin";
...
export const EducationList = () => {
    ...
    const { data, total, isLoading } = useGetList("educations", {
    ...
    const columns = useMemo(
        () => [
            ...
            columnHelper.accessor("startDate", {
                header: "Start",
                cell: (info) => (
                    <span style={{ color: "#6B7280", fontSize: "0.82rem" }}>{formatDate(info.getValue())}</span>
                ),
            }),
            ...
            columnHelper.accessor("isCurrent", {
                header: "Status",
                cell: (info) => <BoolBadge value={info.getValue()} trueLabel="Current" falseLabel="Completed" />,
            }),
            columnHelper.display({
                ...
                cell: ({ row }) => (
                    <button
                        onClick={(e) => {
                            e.stopPropagation();
                            navigate(`/educations/${row.original.id}`);
                        }}
                        style={{
                            padding: "5px 14px",
                            fontSize: "0.78rem",
    ...
    return (
        <div style={{ padding: "28px 32px", width: "100%", boxSizing: "border-box" }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20 }}>
                <h1 style={{ fontSize: "1.4rem", fontWeight: 700, color: "#111827", margin: 0 }}>Education</h1>
                <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                    <span style={{ fontSize: "0.82rem", color: "#9CA3AF" }}>{total ?? 0} total</span>
                    <button
                        onClick={() => navigate("/educations/create")}
                        style={{
                            padding: "8px 16px",
                            fontSize: "0.82rem",
                            fontWeight: 600,
                        ...
                        <tbody>
                            {isLoading ? (
                                ...
                                table.getRowModel().rows.map((row, i) => (
                                    <tr
                                        key={row.id}
                                        onClick={() => navigate(`/educations/${row.original.id}`)}
                                        style={{
...
// ─── Form Fields ──────────────────────────────────────────────────────────────
const EducationFormFields = ({ isCreate = false }) => (
    <div style={{ width: "100%", padding: 32, boxSizing: "border-box" }}>
        <div style={{ marginBottom: 24, paddingBottom: 16, borderBottom: "1px solid #F3F4F6" }}>
            <h2 style={{ fontSize: "0.95rem", fontWeight: 600, color: "#111827", margin: 0 }}>
                {isCreate ? "Add Education" : "Edit Education"}
            </h2>
            <p style={{ fontSize: "0.78rem", color: "#9CA3AF", margin: "4px 0 0" }}>
                {isCreate ? "Enter your education details" : "Update education details below"}
            </p>
        </div>

        {/* Row 1: Institution (full width) */}
        <div style={{ marginBottom: 20, width: "100%" }}>
            <TextInput source="institutionName" validate={required()} label="Institution Name" fullWidth />
        </div>

        {/* Row 2: Degree + Field of Study */}
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20, marginBottom: 20, width: "100%" }}>
            <TextInput source="degree" validate={required()} label="Degree" fullWidth />
            {!isCreate && <TextInput source="fieldOfStudy" label="Field of Study" fullWidth />}
        </div>
        ...
        {/* Row 4: Currently studying toggle (edit only) */}
        {!isCreate && (
            <div
                style={{
                    padding: "12px 16px",
            ...
            >
                <BooleanInput source="isCurrent" label="Currently Studying Here" />
            </div>
        )}
    </div>
);
...
export const EducationEdit = (props) => (
    <Edit {...props} component="div">
        <SimpleForm>
            <FormCard>
                <EducationFormFields isCreate={false} />
            </FormCard>
        </SimpleForm>
    </Edit>
);
...
export const EducationCreate = (props) => (
    <Create {...props} component="div">
        <SimpleForm>
            <FormCard>
                <EducationFormFields isCreate={true} />
            </FormCard>
        </SimpleForm>
    </Create>
);
