...
import { Create, DateInput, Edit, required, SimpleForm, TextInput, useGetList } from "react-admin";
...
export const CertificationList = () => {
    ...
    const { data, total, isLoading } = useGetList("certifications", {
    ...
    const columns = useMemo(
        () => [
            ...
            columnHelper.accessor("issuingOrganization", {
                ...
                cell: (info) => (
                    <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                        ...
                        <span style={{ color: "#374151" }}>{info.getValue()}</span>
                    </div>
                ),
            }),
            columnHelper.accessor("issueDate", {
                ...
                cell: (info) => (
                    <span
                        style={{ color: "#6B7280", fontSize: "0.82rem", display: "flex", alignItems: "center", gap: 6 }}
                    >
            ...
            columnHelper.accessor("credentialUrl", {
                header: "Credential",
                cell: (info) =>
                    info.getValue() ? (
                        <a
            ...
            columnHelper.display({
                ...
                cell: ({ row }) => (
                    <button
                        onClick={(e) => {
                            e.stopPropagation();
                            navigate(`/certifications/${row.original.id}`);
                        }}
                        style={{
                            padding: "5px 14px",
                            fontSize: "0.78rem",
    ...
    return (
        <div style={{ padding: "28px 32px", width: "100%", boxSizing: "border-box" }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20 }}>
                <h1 style={{ fontSize: "1.4rem", fontWeight: 700, color: "#111827", margin: 0 }}>Certifications</h1>
                        ...
                        <tbody>
                            {isLoading ? (
                                ...
                                table.getRowModel().rows.map((row, i) => (
                                    <tr
                                        key={row.id}
                                        onClick={() => navigate(`/certifications/${row.original.id}`)}
                                        style={{
...
// ─── Shared Form Fields ───────────────────────────────────────────────────────
const CertificationFormFields = ({ isCreate = false }) => (
    <div style={{ width: "100%", padding: 32, boxSizing: "border-box" }}>
        <div style={{ marginBottom: 24, paddingBottom: 16, borderBottom: "1px solid #F3F4F6" }}>
            <h2 style={{ fontSize: "0.95rem", fontWeight: 600, color: "#111827", margin: 0 }}>
                {isCreate ? "Add Certification" : "Edit Certification"}
            </h2>
            <p style={{ fontSize: "0.78rem", color: "#9CA3AF", margin: "4px 0 0" }}>
                {isCreate ? "Enter certification details below" : "Update certification details below"}
            </p>
        </div>

        {/* Row 1: Title (full width) */}
        <div style={{ marginBottom: 20, width: "100%" }}>
            <TextInput source="title" validate={required()} label="Certification Title" fullWidth />
        </div>

        {/* Row 2: Org + Issue Date */}
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20, marginBottom: 20, width: "100%" }}>
            <TextInput source="issuingOrganization" validate={required()} label="Issuing Organization" fullWidth />
            <DateInput source="issueDate" label="Issue Date" {...(isCreate ? { validate: required() } : {})} />
        </div>

        {/* Row 3: Credential URL (full width) */}
        <div style={{ width: "100%" }}>
            <TextInput source="credentialUrl" label="Credential URL (optional)" placeholder="https://..." fullWidth />
        </div>
    </div>
);
...
export const CertificationEdit = (props) => (
    <Edit {...props} component="div">
        <SimpleForm>
            <FormCard>
                <CertificationFormFields isCreate={false} />
            </FormCard>
        </SimpleForm>
    </Edit>
);
...
export const CertificationCreate = (props) => (
    <Create {...props} component="div">
        <SimpleForm>
            <FormCard>
                <CertificationFormFields isCreate={true} />
            </FormCard>
        </SimpleForm>
    </Create>
);
